...
The dashboard now consumes the canonical readiness report and renders the documented twelve stages:
...
- [x] Replace the reduced eight-stage dashboard with a representation of all twelve canonical
  stages.
- [x] Show `Setup` as complete when a valid workspace and target configuration exist.
- [x] Show `Authentication` readiness independently for each controlled actor.
- [x] Keep authentication separate from passive-analysis completion so the UI cannot imply that
  credentials are ready merely because hypotheses exist.
- [x] Split `Classification` from `Normalization`, even if both are produced by one workflow call.
- [x] Split `Invariants` from `Model`, while retaining the combined Model view if useful.
- [x] Add an `Execution` stage between Plan and Validate.
- [x] Represent execution with canonical lifecycle state and separate CLI/Web capability metadata,
  without adding browser execution controls.
- [x] Base stage state on artifact validity and prerequisites, not only non-zero record counts.
- [x] Handle stale or out-of-order artifacts without marking later stages complete incorrectly.
- [x] Add tests covering empty, partial, complete, stale, and conflicting pipeline states.
...
- [x] Preserve stable IDs and researcher edits when a stage is regenerated.
- [ ] Display generation conflicts and preserved researcher edits after each run.
- [x] Retain a single `Run complete passive workflow` action for the normal path.
...
- [x] Never render original evidence file bodies in browser responses.
...
- [x] Preserve the distinction between researcher assertions and independently derived facts.
...
- [x] Make clear that validation attempts to disprove or downgrade a hypothesis.
...
- [x] Preserve immutable, versioned report revisions.
- [ ] Preview the validation checksum/state that will be used to generate the report.
- [x] Continue allowing existing report revisions to be read without exposing unrelated files.
...
- [x] Preserve explicit actor/channel assignment for runtime captures.
- [x] Do not infer actor or channel from filenames.
...
- [x] Keep automatic credential capture disabled in the browser by default.
...
- [x] Do not silently make actor authentication `READY` after ordinary Web UI ingestion.
...
- [x] Show suppressed static assets, telemetry, analytics, third-party traffic, public resources,
  and insufficient-evidence dispositions.
- [ ] Add a normalization-anomaly queue for low-confidence opaque path families.
- [x] Show classification and suppression reasons without returning concrete request values.
...
- [x] Continue excluding raw query values, body values, cookies, and authorization headers.
...
- [ ] Show invariant category, source endpoints, evidence links, knowledge status, and validation
  status.
- [x] Improve access to generated authorization, workflow, and state-machine documents.
- [x] Make the distinction between inferred model records and expected invariants explicit.

### Hypotheses

- [ ] Add an independent safe hypothesis-regeneration action.
- [ ] Show suppressed candidates when explicitly requested.
- [x] Preserve a distinct research-task view for leads that fail active-hypothesis evidence gates.
...
- [x] Keep browser execution controls absent unless a separate security design review explicitly
  approves them.
...
- [x] Never provide a generic filesystem editor or arbitrary workspace-path API.
...
- [x] Credential set, import, refresh, refresh-flow configuration, and credential clearing.
- [x] Network-backed actor authentication checks.
- [x] Checksum-bound plan approval and approval-token handling.
- [x] Live target execution.
- [x] Execution confirmation and active-execution authority review.
- [x] Runtime credential resolution or export.
- [x] Any operation that sends requests to a target.
...
- [ ] State clearly that sending the exported request from Burp is manual active execution outside
  Hunt.
- [x] Do not add direct Burp sending or Burp MCP execution to the Web UI.

## Raw Data And Privacy Boundaries

- [x] Continue excluding raw captures from all API responses.
- [x] Continue excluding credential profile references and secret-store keys.
- [x] Continue excluding concrete path, query, header, cookie, and body values where they can contain
  secrets or personal data.
- [x] Continue excluding evidence file bodies by default.
- [ ] If sanitized artifact previews are added, define a second redaction pass and strict content
  limits.
- [x] Keep browser caching disabled and retain the restrictive content security policy.
- [x] Continue loopback-only binding while the server has no authentication.
...
- [x] Require the existing same-origin custom header for every new local write.
- [x] Use strict request models with unknown fields rejected.
...
- [x] Update `docs/web-ui.md`, `docs/manual.md`, and `README.md` as features are added.
- [x] Keep the displayed pipeline aligned with the canonical workflow documentation.
